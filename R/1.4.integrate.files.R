...
#'
#' @param dir_proc Working directory for processing samples.
...
sc_integrate_data <- function( # nolint
  ...
  if (proc_mode == "multiome") {
    cat("Running multiome integration...", "\n")
    if (file.exists(paste0(dir_proc, "03_integration_correct.rds"))) {
      cat("Integrated Seurat object already exists; loading existing object...", "\n") # nolint
      d_int <- readRDS(paste0(dir_proc, "03_integration_correct.rds"))
    }
    if (!file.exists(paste0(dir_proc, "03_integration_correct.rds"))) {
      if (file.exists(paste0(dir_proc, "02_integration_merge.rds"))) {
        cat("Merged Seurat object already exists; loading existing object...", "\n") # nolint
        d_int <- readRDS(paste0(dir_proc, "02_integration_merge.rds"))
      }
      if (!file.exists(paste0(dir_proc, "02_integration_merge.rds"))) {
        if (file.exists(paste0(dir_proc, "02_integration_unify.rds"))) {
          cat("Unified peaks Seurat object already exists; loading existing object...", "\n") # nolint
          ld_int <- readRDS(paste0(dir_proc, "02_integration_unify.rds"))
        }
        if (!file.exists(paste0(dir_proc, "02_integration_unify.rds"))) {
          ...
          cat("ATAC peaks unified; saving Seurat object in processing directory", "\n") # nolint
          saveRDS(ld_int, paste0(dir_proc, "02_integration_unify.rds"))
          gc(reset = TRUE)
        }
        print("---- Step 3: WNN integration of individual sample GEX/ATAC assays ----") # nolint
        if (Sys.info()[["sysname"]] != "Windows" && parl == TRUE) {
          ld_int <- setNames(
            parallel::mclapply(
              mc.cores = 2,
        ...
        if (length(ld_int) == 1) {
          ...
          d_int <- ld_int[[1]]
          saveRDS(d_int, paste0(dir_proc, "02_integration_merge.rds"))
        }
        if (length(ld_int) > 1) {
          d_int <- merge(
            x = ld_int[[1]],
      ...
      d_int <- Seurat::SCTransform(d_int)
      gc(reset = TRUE)
      ...
      saveRDS(d_int, paste0(dir_proc, "03_integration_correct.rds"))
      gc(reset = TRUE)
    }
    ...
    cat("Saving final integrated data in current working directory...", "\n") # nolint
    saveRDS(d_int, "data_wnn_integration.rds")
    return(d_int)
  }

  # Standard scRNA-Seq data integration
  if(proc_mode == "scRNA-Seq") { # nolint
    cat("Running scRNA-Seq integration...", "\n")
    if (unlist(packageVersion("Seurat"))[1] == 5) { # nolint
      ...
      if (!file.exists(paste0(dir_proc, "02_integration_merge.rds"))) { # nolint
        ld_int <- l_so
